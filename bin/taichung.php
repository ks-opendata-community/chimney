...
$tmpPath = $rootPath . '/tmp/taichung';
$dataPath = $rootPath . '/data/daily/taichung';
$factories = array('B2402442', 'L0056153', 'L0200473', 'L0200633', 'L9101748', 'L9200693', 'L9200728', 'L9201289',);
$items = array(
    1 => '911', //不透光率 (%)
    2 => '922', //二氧化硫 (ppm)
    3 => '923', //氮氧化物 (ppm)
...
// ex. http://220.130.204.202/program/History/Show_Measure_Detail.asp?Date=1041201&Cno=B2402442

$currentTime = time();
$timeEnd = strtotime('2015-11-16');
$baseUrl = 'http://220.130.204.202/program/History/Show_Measure_Detail.asp?Date=';

while ($currentTime >= $timeEnd) {
    $dayPath = $tmpPath . date('/Y/m/d', $currentTime);
    if (!file_exists($dayPath)) {
        mkdir($dayPath, 0777, true);
    }
    $targetPath = $dataPath . date('/Y/m', $currentTime);
    if (!file_exists($targetPath)) {
        mkdir($targetPath, 0777, true);
    }
    $targetFile = $targetPath . date('/Ymd', $currentTime) . '.csv';
    $dayUrl = $baseUrl . (date('Y', $currentTime) - 1911) . date('md', $currentTime);
    $timeIndexed = array();
        ...
        foreach ($data AS $pol => $lines) {
            foreach ($lines AS $line) {
                if (count($line) === 12) {
                    ...
                    foreach ($items AS $itemKey => $itemCode) {
